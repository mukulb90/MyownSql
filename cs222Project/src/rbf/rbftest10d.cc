...
int RBFTest_10(RecordBasedFileManager *rbfm) {
    ...
    const string queryParam = "a";
    int length = queryParam.size();
    char * data = (char*) malloc(sizeof(int) + sizeof(char)*queryParam.size());
    memcpy(data, &length, sizeof(int));
    const char * query_pointer = queryParam.c_str();
    data += sizeof(int);
    memcpy(data, query_pointer, sizeof(char)*length);

    RBFM_ScanIterator* iterator = new RBFM_ScanIterator();
    rbfm->scan(fileHandle, recordDescriptor, compareAttributeName , LE_OP, (void*)query_pointer , projectedAttributes, *iterator);
